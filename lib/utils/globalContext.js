...
export function GlobalContextWrapper({ children }) {
	...
	async function signIn() {
		...
		await fetch('https://www.devdiary.live/api/addUser', {
			method: 'POST',
		...
		const res = await fetch('https://www.devdiary.live/api/getUser', {
			method: 'POST',
		...
		router.replace('https://www.devdiary.live')
	}
	...
	function signOut() {
		...
		window.location.replace('https://www.devdiary.live')
	}

	async function updateUserName(user_email, userName) {
		const res = await fetch('https://www.devdiary.live/api/updateUserName', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
