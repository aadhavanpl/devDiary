...
export function GlobalContextWrapper({ children }) {
	...
	async function signIn() {
		...
		fetch('https://devdiary.live/api/addUser', {
			method: 'POST',
		...
		const res = await fetch('https://devdiary.live/api/getUser', {
			method: 'POST',
	...
	function signOut() {
		...
		window.location.replace('https://devdiary.live')
	}

	async function updateUserName(user_email, userName) {
		const res = await fetch('https://devdiary.live/api/updateUserName', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
